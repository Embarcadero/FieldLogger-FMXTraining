...
{$REGION ' Log Data'}
  ...
  ILogData = interface
    ...
    /// </summary>
    function Read(out Entries: TArrayOfLogEntry;
      PROJECT_ID: uint32 = 0): integer;
...
uses
  fieldlogger.projectdata.standard, fieldlogger.logdata.standard;
...
begin
  Result := fieldlogger.logdata.standard.TLogData.Create(Connection,
    ValidConnection);
...
var
  ValidConnection: boolean;
begin
  Result := fieldlogger.projectdata.standard.TProjectData.Create(Connection,
    ValidConnection);
  if not ValidConnection then
  begin
    Result := nil;
